...
namespace YouTube
{
    ...
    public class Channel
    {
        ...
        /// <summary>
        /// Get/Set the Type of Youtube Type aka Kind Value
        /// </summary>
        [XmlElement(ElementName = "type")]
        public YouTubeType Type { get; set; }
        /// <summary>
        /// Get/Set the Youtube unique identifer assigned to feed
        /// </summary>
        [XmlElement(ElementName = "youtubeid")]
        ...
        internal Channel(string pconstring, DataRow dr)
        {
            try
            {
                ...
                if (dr["ChnlType"] != System.DBNull.Value) { Type = (YouTubeType)dr["ChnlType"]; }
                if (dr["ChnlYouTubeId"] != System.DBNull.Value) { YouTubeID = (String)dr["ChnlYouTubeId"]; }
            }
            catch (Exception ex)
            {
                GetLastError = ex;
            ...
            using (MySqlConnection sqlconn = new MySqlConnection(pconstring))
            {
                using (MySqlCommand sqlcomm = new MySqlCommand("", sqlconn))
                {
                    ...
                    try
                    {
                        ...
                        query.Append("SELECT * FROM `vw_youtube_channels`");
        ...
        public bool Add(string pconstring)
        {
            using (MySqlConnection sqlconn = new MySqlConnection(pconstring))
            {
                using (MySqlCommand sqlcomm = new MySqlCommand("usp_addYouTubeChannel", sqlconn))
                {
                    ...
                    try
                    {
                        ...
                        sqlcomm.Parameters.Add(new MySqlParameter("?ptype", MySqlDbType.Int16)).Value = Type;
                        sqlcomm.Parameters.Add(new MySqlParameter("?pyoutubeid", MySqlDbType.VarChar, 50)).Value = YouTubeID;
        ...
        public bool Update(string pconstring)
        {
            using (MySqlConnection sqlconn = new MySqlConnection(pconstring))
            {
                using (MySqlCommand sqlcomm = new MySqlCommand("usp_updateYouTubeChannel", sqlconn))
                {
                    sqlcomm.CommandType = System.Data.CommandType.StoredProcedure;
                    try
                    {
        ...
        public Boolean Delete(String pconstring)
        {
            using (MySqlConnection sqlconn = new MySqlConnection(pconstring))
            {
                using (MySqlCommand sqlcomm = new MySqlCommand("usp_deleteYouTubeChannel", sqlconn))
                {
                    sqlcomm.CommandType = System.Data.CommandType.StoredProcedure;
                    try
                    {
        ...
        public bool getFeed(int maxRecords = 10)
        {
            try
            {
                ...
                for (int i = 0; i <= maxRecords; i++)
                {
                    ...
                    XmlNode node;
                    node = rssList.Item(i).SelectSingleNode("guid");
                    if (node != null)
                        it.YouTubeGuid = node.InnerText;
                    else
                    {
                        it.YouTubeGuid = "";
                    }
                    ...
                    node = rssList.Item(i).SelectSingleNode("category");
                    if (node != null)
                        it.Category = node.InnerText;
                    else
                    {
                        it.Category = "";
                    }
                    ...
                    node = rssList.Item(i).SelectSingleNode("author");
                    if (node != null)
                        it.Author = node.InnerText;
                    else
                        it.Author = "";
        ...
        /// </summary>
        /// <returns></returns>
        public string ItemsToEmbededVideos()
        {
            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            ...
            if (Items != null && Items.Count > 0)
            {
                foreach (Item i in Items)
                {
                    sb.Append(string.Format("<li class='' style='display:inline-block;'>"));
                    sb.Append(string.Format("<iframe width='560' height='315' src='{0}' frameborder='0' allowfullscreen></iframe>", "https://www.youtube.com/embed/lEXBxijQREo"));
        ...
        /// <summary>
        /// Return DataTable of all YouTube from system
        /// </summary>
        /// <param name="pconstring">as data source connection string</param>
        /// <returns></returns>
        /// <TableName>[vw_youtube_channels]</TableName>
